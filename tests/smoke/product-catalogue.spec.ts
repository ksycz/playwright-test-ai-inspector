...
test.describe('M3 — Product Catalogue', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('/products');
  });

  test('M3-01: catalogue displays all products from JSON', async ({ page }) => {
    await expect(page.getByRole('heading', { name: 'Products', level: 1 })).toBeVisible();
    await expect(page.getByRole('article')).toHaveCount(products.length);
  });

  test('M3-02: product card shows name, category, price, and description', async ({ page }) => {
  ...
  test('M3-03: add to cart from catalogue updates badge to 1', async ({ page }) => {
    await page.getByRole('button', { name: `Add ${sampleProduct.name} to cart` }).click();

    await expect(page.getByRole('link', { name: 'Cart, 1 item' })).toBeVisible();
  });

  test('M3-04: adding same product twice updates badge to 2', async ({ page }) => {
  ...
  test('M3-05: product images have alt text matching product name', async ({ page }) => {
