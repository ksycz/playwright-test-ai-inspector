...
test.describe('M5 — Shopping Cart', () => {
  test('M5-01: cart shows added items with name, quantity, and line subtotal', async ({ page }) => {
  ...
  test('M5-02: increase quantity updates line subtotal and cart total', async ({ page }) => {
    await addSampleProductToCart(page);
  ...
  test('M5-03: decrease quantity to 0 removes the item', async ({ page }) => {
    await addSampleProductToCart(page);
  ...
  test('M5-04: remove item deletes it and recalculates total', async ({ page }) => {
    await addSampleProductToCart(page);
  ...
  test('M5-05: clear cart empties all items', async ({ page }) => {
    await addSampleProductToCart(page);
    await page.goto('/products');
  ...
  test('M5-06: empty cart shows message and link to Products', async ({ page }) => {
    await page.goto('/cart');
  ...
  test('M5-07: cart persists after page reload', async ({ page }) => {
    await addSampleProductToCart(page);
  ...
  test('M5-08: proceed to checkout button is visible', async ({ page }) => {
    await addSampleProductToCart(page);
